global_IPD_loss <- function(fit.result, local_lst, gamma, lambda1, lambda2){
  loss.result = sum(sapply(1:dim(fit.result)[2], function(m){
    nrow(local_lst[[m]]$dat.obs) *
      log_lkh(eta = fit.result[stringr::str_detect(rownames(fit.result),"^psi"),m],
              xi =  as.matrix(fit.result[stringr::str_detect(rownames(fit.result),"^phi[0-9]+$"),m],
                              ncol = 1),
              beta =  as.matrix(fit.result[!stringr::str_detect(rownames(fit.result),"^phi[0-9]+$") &
                                             !stringr::str_detect(rownames(fit.result),"^psi"),m],
                                ncol = 1),
              dat.obs = local_lst[[m]]$dat.obs,
              psi = local_lst[[m]]$psi,
              d_phi = sum(stringr::str_detect(rownames(fit.result),"^phi[0-9]$")),
              knot = local_lst[[m]]$psi.knot
      )
  }))

  pen1 = fit.result[stringr::str_detect(rownames(fit.result),"^phi[0-9]+\\."), ]
  pen1.sub.sum = c()
  for(phi.j in 1:sum(stringr::str_detect(rownames(fit.result),"^phi[0-9]$"))){
    pen1.sub = pen1[stringr::str_detect(rownames(pen1),as.character(phi.j)),]
  ...
  pen2 = fit.result[!stringr::str_detect(rownames(fit.result),"^phi") &
                      !stringr::str_detect(rownames(fit.result),"^psi"),1]
...
eval_fun <- function(dat_obj, local_est, theta_fit){
  ...
  # Calculate the error of the predicted risk (lp, linear predictor since required by survAUC::AUC.sh)
  ...
  auc_eval <- survAUC::AUC.sh(Surv.rsp = survival::Surv(dat_obj$y[train_indx], dat_obj$failed[train_indx]),
                     Surv.rsp.new = survival::Surv(dat_obj$y[test_indx], dat_obj$failed[test_indx]),
                     lp = pred_val[train_indx], lpnew = pred_val[test_indx],
                     times = seq(min(dat_obj$y), stats::median(dat_obj$y), length.out = 10))
