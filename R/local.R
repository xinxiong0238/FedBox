...
surv_local <- function(local_raw, num_bh_knot=2, d_phi=3, n.nodes=15){
  ...
  x <- local_raw[, str_detect(colnames(local_raw), "X")]
  time <- local_raw$y
...
get_local_est <- function(dat.obs, num_bh_knot, d_phi, n.nodes = 15){
  ...
  xdata <- dplyr::select(dat.obs, -all_of(nonx_col))
  ...
  ridge_model <- cv.glmnet(x = as.matrix(xdata),
                          y = Surv(dat.obs$y, dat.obs$failed),
                          family = 'cox',
                          alpha = 0,
  ...
  mymodel <- as.formula(paste0('Surv(y, failed) ~', 
                               paste0(colnames(xdata), collapse='+')))
  cox_model <- coxph(mymodel, data = dat.obs)
  cox_model$coefficients <- coef(alasso_model, s = 'lambda.min')
  
  # extract beta
  coeff_theta <- coef(alasso_model, s = 'lambda.min')
