...
generate_data_Multilocals <- function(seed = 123, n_vec, T_vec,
  ...
  betas <- data.frame("coeff_name" = betas_name,
                      "coeff" = c(stats::runif(round(beta_nonzero_num/2), min = -2, max = -0.1),
                                  stats::runif(beta_nonzero_num - round(beta_nonzero_num/2),
                                                                 min = 0.1, max = 2),
                                  stats::runif(beta_nonzero_num, min = -0.3, max = 0.3),
                                  stats::runif(beta_nonzero_num, min = -2, max = 2),
                                  stats::runif(beta_nonzero_num, min = -0.4, max = 0.4)
                                  )
                      )
  ...
  for(k in 1:K){
    T0 <- T_vec[k]
    n_patient <- n_vec[k]
    beta_noise <- stats::rnorm(ncol(betas), 0, 0.1)
    beta_noise[1:beta_nonzero_num] = 0
    rho <- stats::runif(1,0.3,0.6)
...
generate_data = function(n_patient, n_beta, betas, T0,
  ...
  X0 <- MASS::mvrnorm(n_patient, mu = rep(0, n_beta), Sigma = Xcov) # mvn with cov=ar(1)
  ...
  for (j in 1:n_beta){
    miss_indx <- X0[,j] < 1
    X[miss_indx, paste0('M',j)] <- stats::rbinom(sum(miss_indx),1,missing_rate[j])
...
simul <- function(N, X, betas, lambda, rho, beta, dist_family='weibull'){
  ...
  # censoring times
  C = stats::runif(n = N,stats::quantile(Tlat,0.5),
                   stats::quantile(Tlat,0.9))
