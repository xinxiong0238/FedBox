  ...
  for (lambda1 in lambda1_lst) {
    for (lambda2 in lambda2_lst) {
      ...
      while(loss.change < 0){
        ...
        for(m in 1:M){
          local_lst_new[[m]]$eta =
            fit.result.new[stringr::str_detect(rownames(fit.result.new),"^psi"),m]
          local_lst_new[[m]]$xi =
            as.matrix(fit.result.new[stringr::str_detect(rownames(fit.result.new),"^phi[0-9]+$"),m],
                      ncol = 1)
          local_lst_new[[m]]$beta =
            as.matrix(fit.result.new[!stringr::str_detect(rownames(fit.result.new),"^phi[0-9]+$") &
                                   !stringr::str_detect(rownames(fit.result.new),"^psi"),m],
                      ncol = 1)
        ...
        # fit.change = (norm(fit.result[!stringr::str_detect(rownames(fit.result),"^phi[0-9]+$") &
        #                                 !stringr::str_detect(rownames(fit.result),"^psi"),] -
        #                      fit.result[!stringr::str_detect(rownames(fit.result),"^phi[0-9]+$") &
        #                      !stringr::str_detect(rownames(fit.result),"^psi"),], type = '2'))^2
        #
        #
