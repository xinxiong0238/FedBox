...
pre_inte_XY <- function(H_lst, g_lst, theta_lst, n_lst,
  ...
  for (m in 1:M){
    ...
    if (m >= 2){
      X_all_right <- Matrix::bdiag(X_all_right, X_lst[[m]][,which(share_var == 0)])
    }
...
integrative_fit_for_cv <- function(X_all, Y_all, M, share_var, penalty.factor.lambda,
  ...
  fit.init <- glmnet::glmnet(X_all, Y_all, intercept = F, standardize = F, lambda = 0)
  ...
  while (error_delta > tol & iter < max.iter) {
    ...
    fit.m <- glmnet::glmnet(X_all[,indx_use], Y_all, intercept = F, standardize = F,
...
Cal_GIC <- function(H_lst, g_lst, theta_lst, n_lst, share_var, theta_mat, penalty.factor,
  ...
  for (m in 2:M){
    Design_mat_all <- Matrix::bdiag(Design_mat_all,
                            n_lst[m] * H_lst[[m]][indx_diff,indx_diff])
    W_mat_all <- Matrix::bdiag(W_mat_all,
